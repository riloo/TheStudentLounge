...
<style>
body {
	font-family: Arial, Helvetica, sans-serif; width 80%; height 80% margin
	: 50px;
	margin-right: auto;
	margin-left: auto;
	padding: 0 10px;
		...
		ajaxRequest.onreadystatechange = function() {
			...
			if (ajaxRequest.readyState == 4) {
				...
				if (ajaxRequest.status == 200) {
					    ...
					    var doc = ajaxRequest.response;
					    var elem = doc.getElementById("myBox");
					    //alert(elem.innerHTML);		
					    document.getElementById("myBox").innerHTML = elem.innerHTML;
					...
					var content = xmlDoc.getElementById("myBox");
					window.alert("my xmldoc. ");
					window.alert(xmlDoc);
					if(xmlDoc == null){window.alert("its null");}
					else{window.alert("its not null");}
					window.alert("mybox");
					window.alert(content);
					document.getElementById("myBox").innerHTML = content;*/
					//document.getElementById("myBox").innerHTML = ajaxRequest.responseText.getElementsById("myBox");
					//document = ajaxRequest.responseText;
				} else {
...
<a class = "buttonlink" href="./getAllRoomsServlet">My rooms &nbsp</a>
	...
	<h2>Chat Messages</h2>
			...
			<%
					...
					String myID = request.getSession().getAttribute("userID").toString();
					String color;
					for(Message mess : msgs)
					{
				
						if(mess.getUserId().compareTo(myID) == 0){
							color = "#547ef2";
						}
						else{
							color = "#e6e6e6";
			...
			%>
			<tr class="container" style="background-color:<%=color%>">
				<td><%=mess.getFname()%>:</td>
				<td><%=mess.getMessage()%></td>
				<td class="time-right"><%=mess.getNormalizedDate()%></td>
			</tr>
			<%
	...
	<form action="./PostMessage" method="POST"
		onsubmit="return clearOnSubmission">

		<div class="page_bottom">

			<div class="example">
				<input type="text" placeholder="Enter message here " name="userPost"
					required>
				<button class="example-button" type="submit">
